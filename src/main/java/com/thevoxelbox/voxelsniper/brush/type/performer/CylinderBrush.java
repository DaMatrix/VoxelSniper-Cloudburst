	...
	public void handleCommand(String[] parameters, Snipe snipe) {
		...
		for (String parameter : parameters) {
			if (parameter.equalsIgnoreCase("info")) {
				messenger.sendMessage(TextFormat.GOLD + "Cylinder Brush Parameters:");
				messenger.sendMessage(TextFormat.AQUA + "/b c h[number] -- set the cylinder v.voxelHeight.  Default is 1.");
				messenger.sendMessage(TextFormat.DARK_AQUA + "/b c true -- will use a true circle algorithm instead of the skinnier version with classic sniper nubs. /b b false will switch back. (false is default)");
				messenger.sendMessage(TextFormat.DARK_BLUE + "/b c c[number] -- set the origin of the cylinder compared to the target block. Positive numbers will move the cylinder upward, negative will move it downward.");
				return;
			}
			if (parameter.startsWith("true")) {
				...
				messenger.sendMessage(TextFormat.AQUA + "Cylinder v.voxelHeight set to: " + toolkitProperties.getVoxelHeight());
				...
				messenger.sendMessage(TextFormat.AQUA + "Cylinder origin set to: " + toolkitProperties.getCylinderCenter());
			} else {
